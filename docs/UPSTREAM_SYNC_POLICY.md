...
8. 核心测试、品牌契约、页面流程、视觉快照或受保护路径检查任一失败，都不得合并。
...
UI 实施阶段应增加自动化品牌契约，至少检查：

- 产品名和 Slogan 存在；
- 活跃页面 Canonical 使用 `https://01yapi.cc`；
- 活跃模板不使用 `veridrop.org` 作为当前站点身份；
- 不包含上游 Google Analytics ID；
- 关键 DOM Hook、表单字段、API Endpoint 和报告链接仍存在；
- 首页和关键页面视觉快照没有未批准变化。
