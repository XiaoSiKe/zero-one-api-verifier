...
**零一智鉴 · API 真测雷达**<br>
从零到一，让每一个接口有据可鉴！<br>
正式域名：[mix.01yapi.cc](https://mix.01yapi.cc)
...
顶部“访客数据”进入 `/partner/admin?section=visits`，展示近 7 天浏览量和独立访客、近 14 天访问与站长申请趋势、访问最多的页面、待审申请、已收录站点及展示中广告位。统计从部署后开始累计：公共 FastAPI 页面由服务端计数，React 首页通过 `POST /api/visit` 计数；同一浏览器同一页面 5 秒内重复请求去重，管理员访问和常见爬虫/CLI 不计。数据保存在 `web_data/visits.sqlite3`，仅有日期、页面类别、随机访客标识的 SHA-256 哈希和时间，不记录 IP、查询参数或浏览器指纹；访客 Cookie 有效期 30 天，旧访问记录在后续写入时清理至约 90 天。访客数是浏览器估算，不是精确人数，也不代表收入。系统尚未接入在线收款或广告点击归因。

当前运维能力是本地/单实例基础版：登录按 IP 与账号双重限速，伙伴页禁止跨站表单、禁止缓存，并设置 HttpOnly/SameSite Cookie 与基本安全响应头；密码散列使用 PBKDF2-SHA256。事件记录用于核查，不等于集中式安全监控。公开部署前仍需 HTTPS、可信反向代理、外部告警与管理员多因素认证；在线收款、真实广告展示/点击统计尚未接入。

站长工作台沿用首页的悬浮导航与深浅色背景：顶部提供“我的站点、广告订阅、联系、首页”和账号头像菜单。`/partner/sites` 展示站点资料、审核状态与广告数据；`/partner/sites/new` 提交新域名。`/partner/pricing`、`/partner/featured`、`/partner/ads` 保留为兼容入口。商业功能未接入实际支付或投放数据时会明确标注，不显示虚构数字。

新提交的站点必须选择 QQ 或微信并填写联系账号；这些联系方式只供该站长与管理员在站长中心查看，不进入公开报告或榜单。旧收录记录会保留，联系方式字段为空。
...
零一智鉴的当前源码托管在 [XiaoSiKe/zero-one-api-verifier](https://github.com/XiaoSiKe/zero-one-api-verifier)；
