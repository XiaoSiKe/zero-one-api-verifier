...
_STATIC_SITEMAP_URLS = [
    ("https://mix.01yapi.cc/",            "weekly",  "1.0",  "hub.html"),
    ("https://mix.01yapi.cc/claude",      "weekly",  "0.9",  "index.html"),
    ("https://mix.01yapi.cc/openai",      "weekly",  "0.9",  "openai.html"),
    ("https://mix.01yapi.cc/gemini",      "weekly",  "0.9",  "gemini.html"),
    ("https://mix.01yapi.cc/leaderboard", "daily",   "0.85", "leaderboard.html"),
    ("https://mix.01yapi.cc/faq",         "monthly", "0.8",  "faq.html"),
]
...
async def sitemap_xml() -> Response:
    ...
    for dir_path in _SITEMAP_REPORT_DIRS:
        ...
        for json_path in sorted(dir_path.glob("*.json")):
            ...
            try:
                ...
                continue
            lines.append(
                f"  <url><loc>https://mix.01yapi.cc/r/{job_id}</loc>"
    ...
    for r in relays:
        if not leaderboard.is_valid_domain(r.domain):
            continue
        line = f"  <url><loc>https://mix.01yapi.cc/leaderboard/{r.domain}</loc>"
        if r.last_checked:
